...
import { startLoopAuth, cancelOrder } from './tool';
import { backSell, getId } from './tool_v1';
import { useStorage } from '@extension/shared';
import { bicycleSettingStorage, settingStorage } from '@extension/storage';
...
export const BicycleMode = ({
  ...
  const getOptions = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!data.count || !data.type || !data.runNum || !data.checkPriceCount || !data.checkPriceTime) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    for (let i = 0; i < runNum; i++) {
      ...
      // 兜底卖出
      await backSell(tab, symbol, timeout);
    }
  ...
  //     });
  //   }
  //   const symbol = await getId(tab).catch(() => '');
  //   for (let i = 0; i < runNum; i++) {
  //     if (isStop) {
  //       break;
  //     }
  ...
  //         const check = await checkByOrderSell(tab, Number(data.checkPriceTime)).catch(err => {
  ...
  //       while (flow <= count) {
  //         const buyPrice = await getPrice(tab, data.type, symbol);
  ...
  //       const nextPrice = await getPrice(tab, data.type, symbol);
